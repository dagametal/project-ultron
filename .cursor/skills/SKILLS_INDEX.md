...
| QA: Cierre | Cierra QA de una HU AVCD: subtarea QA: Cierre, TE/Tests a Terminado, runs TO DO/EXECUTING a PASSED, comentario Evidencias con smart link, subtarea Terminado y HU en validacion po. | El usuario dice `qa-cierre`, QA: Cierre, cierre de HU, cerrar certificación, o pide crear QA: Cierre y evidencias de una HU. | Key de HU (`AVCD-XXXX` o `XXXX`); si hay varias TE, `--te`; credenciales en `jira-config.js` (`EMAIL`, `API_TOKEN`, `XRAY_CLIENT_ID`, `XRAY_CLIENT_SECRET`). |
